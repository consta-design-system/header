...
Ряд бейджиков, красиво сворачивается, если не помещается по длине в родительский контейнер. Наследует свойства компонента [Badge](http://uikit.gizeasy.ru/?path=/docs/components-badge--playground).
...
- `status` — статус бейджика, варианты: `'success'` , `'error'` , `'warning'`, `'normal'`, `'system'` (подрбонее см. [Badge](http://uikit.gizeasy.ru/?path=/docs/components-badge--playground)),
- `icon` — любая [иконка](http://uikit.gizeasy.ru/?path=/story/components-icons--playground).
