...
- `status` — статус бейджика, варианты: `'success'`, `'error'`, `'warning'`, `'normal'`, `'system'` (см. [Badge](http://uikit.gizeasy.ru/?path=/docs/components-badge--playground)). По умолчанию `'normal'`.
...
Если укажете `view='user'`, уведомление будет вести себя как компонент [User](http://uikit.gizeasy.ru/?path=/docs/components-user--playground): `title` превратится в имя пользователя, а `image` — в его аватарку.
...
- `icon` — иконка из [библиотеки иконок](http://uikit.gizeasy.ru/?path=/story/components-icons--playground),
