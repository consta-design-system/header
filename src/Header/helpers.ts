...
import { getGetters as getGettersForButtonMenu } from '@/ButtonMenu/helpers'
import { getGetters as getGettersForSelectMenu } from '@/SelectMenu/helpers'
  ...
  return [
    {
      ...
      subMenu: props.secondaryMenu.map(item => ({
        ...
        href: getters.getItemHref(item),
        target: getters.getItemTarget(item),
        onClick: getItemClick(item, getters.getItemOnClick, props.onSecondaryMenuItemClick),
        subMenu: getters.getItemSubMenu(item),
      })),
    },
  ]
...
const getAdditionalButtons = (props: HeaderProps): DefaultItem[] => {
  ...
  return props.additionalButtons.map(item => ({
    ...
    href: getters.getItemHref(item),
    target: getters.getItemTarget(item),
    onClick: getItemClick(item, getters.getItemOnClick, props.onAdditionalButtonsItemClick),
    groupId: '2',
  }))
}

const getLanguages = (props: HeaderProps, state: HeaderState): DefaultItem[] => {
  ...
  const getActive = (item: DefaultItem): boolean => {
    ...
    return getters.getItemLabel(item) === getters.getItemLabel(state.languagesValue)
  }
...
export const getMobileMenu = (props: HeaderProps, state: HeaderState): DefaultItem[] => [
  ...getMenu(props),
