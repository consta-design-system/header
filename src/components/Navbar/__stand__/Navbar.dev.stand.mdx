...
import { NavbarControlledExample } from './examples/NavbarControlledExample/NavbarControlledExample';
import {
  NavbarRailExample,
...
const onSubMenuToggle = (item, { open, e }) => {
...
type NavbarPropGetItemSubMenuOpen<ITEM> = (item: ITEM) => boolean;

type NavbarProponOnSubMenuToggle<ITEM> = (
  item: ITEM,
  params: { open: boolean; e?: React.MouseEvent },
) => void;
```
...
| `onSubMenuToggle?`            | `NavbarProponOnSubMenuToggle<ITEM>`           | -                            | Функция для обработки изменения состояния подменю элемента                   |
