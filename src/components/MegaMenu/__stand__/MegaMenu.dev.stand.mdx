...
За данные пунктов меню отвечает свойство `items` (тип [MegaMenuDefaultItem](#свойства)). В него входит:
...
Если поля `subMenu` у элементов нет, то будет ренериться только [`NavBar`](##LIBS.LIB.STAND/lib:header/stand:components-navbar-canary), если есть и уровень вложенности 1, то будет рендериться [`NavBar`](##LIBS.LIB.STAND/lib:header/stand:components-globalmenu-canary), иначе оба компонента сразу.
...
| ----------------------- | --------------------------------- | ----------------------------- | ----------------------------------------------------------------------- |
| `isOpen?`               | `boolean`                         | -                             | Флаг открыто ли меню                                                    |
...
| `onItemClick?`          | `MegaMenuPropOnItemClick`         | -                             | Событие по нажаьию на элемент                                           |
| `onBannerClick?`        | `MegaMenuPropOnItemClick`         | -                             | Событие по нажаьию на баннер                                            |
...
| `getItemAs?`            | `MegaMenuPropGetItemAs`           | `defaultGetItemAs`            | Функция для определения тэга элемента                                   |
...
| `getBannerLabel?`       | `BannerBarPropGetItemLabel`       | `defaultGetBannerLabel`       | Функция для определения заголовка элемента                              |
| `getBannerDescription?` | `BannerBarPropGetItemDescription` | `defaultGetBannerDescription` | Функция для определения заголовка элемента                              |
| `getBannerImage?`       | `BannerBarPropGetItemImage`       | `defaultGetBannerImage`       | Функция для определения заголовка элемента                              |
| `getBannerOnClick?`     | `BannerBarPropGetItemOnClick`     | `defaultGetBannerOnClick`     | Функция для определения заголовка элемента                              |
| `getBannerAs?`          | `BannerBarPropGetItemAs`          | `defaultGetBannerAs`          | Функция для определения заголовка элемента                              |
| `getBannerAttributes?`  | `BannerBarPropGetItemAttributes`  | `defaultGetBannerAttributes`  | Функция для определения заголовка элемента                              |
