...
- `status` — статус бейджика, варианты: `'success'`, `'error'`, `'warning'`, `'normal'`, `'system'` (см. [Badge](##LIBS.STAND|stand:uikit-components-badge-stable)). По умолчанию `'normal'`.
...
Если укажете `view='user'`, уведомление будет вести себя как компонент [User](##LIBS.STAND|stand:uikit-components-user-stable): `title` превратится в имя пользователя, а `image` — в его аватарку.
...
- `icon` — иконка из [библиотеки иконок](##LIBS.STAND|stand:uikit-components-icon-stable),
