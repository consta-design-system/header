...
- `status` — статус бейджика, варианты: `'success'` , `'error'` , `'warning'`, `'normal'`, `'system'` (подрбонее см. [Badge](##LIBS.LIB.STAND/lib:uikit/stand:components-badge-stable)),
- `icon` — любая [иконка](##LIBS.LIB.STAND/lib:uikit/stand:components-icons-stable).
