...
import {
  ...
  ButtonMenuExample50,
} from './examples/ButtonMenuExample/ButtonMenuExample';
import { ButtonMenuExampleForm } from './examples/ButtonMenuExampleForm/ButtonMenuExampleForm';
import { ButtonMenuExampleSize } from './examples/ButtonMenuExampleSize/ButtonMenuExampleSize';
import { ButtonMenuExampleIcon } from './examples/ButtonMenuExampleIcon/ButtonMenuExampleIcon';
import { ButtonMenuExampleOnlyIcon } from './examples/ButtonMenuExampleOnlyIcon/ButtonMenuExampleOnlyIcon';
import { ButtonMenuExampleView } from './examples/ButtonMenuExampleView/ButtonMenuExampleView';
...
type ButtonMenuPropOnItemClick<ITEM> = (
  item: ITEM,
  props: {
    e: React.MouseEvent;
  },
...
| [`getItemHref?`](#пункты-меню)    | `ButtonMenuPropGetItemHref`                                                               | `defaultGetItemHref`    | Функция для определения `href` элемента                                                 |
| [`getItemLabel?`](#пункты-меню)   | `ButtonMenuPropGetItemLabel`                                                              | `defaultGetItemLabel`   | Функция для определения заголовка элемента                                              |
| [`getItemTarget?`](#пункты-меню)  | `ButtonMenuPropGetItemTarget`                                                             | `defaultGetItemTarget`  | Функция для определения `target` элемента                                               |
| [`getItemOnClick?`](#пункты-меню) | `ButtonMenuPropGetItemOnClick`                                                            | `defaultGetItemOnClick` | Функция для определения функции клика элемента                                          |
| [`getItemIcon?`](#иконка)         | `ButtonMenuPropGetItemIcon`                                                               | `defaultGetItemIcon`    | Функция для определения иконки элемента                                                 |
| [`form?`](#форма)                 | `'default', 'round', 'brick', 'brickRound', 'roundBrick', 'brickDefault', 'defaultBrick'` | `'default'`             | Форма (см. [Button](##LIBS.LIB.STAND/lib:uikit/stand:components-button-stable))         |
| [`size?`](#размер)                | `'xs', 's', 'm', 'l'`                                                                     | `'m'`                   | Размер (см. [Button](##LIBS.LIB.STAND/lib:uikit/stand:components-button-stable))        |
| [`view?`](#внешний-вид)           | `'clear', 'primary', 'secondary', 'ghost'`                                                | `'primary'`             | Вид (см. [Button](##LIBS.LIB.STAND/lib:uikit/stand:components-button-stable))           |
| [`onlyIcon?`](#иконка)            | `boolean`                                                                                 | -                       | Только иконка (см. [Button](##LIBS.LIB.STAND/lib:uikit/stand:components-button-stable)) |
